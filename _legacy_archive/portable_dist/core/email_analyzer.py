...
class EmailAnalyzer:
    def __init__(self, config_file='ai_config.json'):
        ...
        # Use provider-specific timeout, fallback to root timeout, fallback to 300
...
"""
    ...
    def extract_wiki_entities(self, subject, body):
        ...
        # Load schema categories for prompt
        categories_desc = ""
        try:
            ontology_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'schema', 'ontology.json')
            if os.path.exists(ontology_path):
                with open(ontology_path, 'r', encoding='utf-8') as f:
                    ontology = json.load(f)
                    for i, (cat, desc) in enumerate(ontology.get('categories', {}).items()):
                        categories_desc += f"{i+1}. {cat:8} → {desc}\n"
        except: pass
        
        if not categories_desc:
            # Fallback if file missing
            categories_desc = "1. org, 2. man, 3. project, 4. method, 5. measure, 6. material, 7. machine, 8. env, 9. artifact, 10. domain, 11. event"
...
2. DO NOT just append. Rewrite the "Description" or "Status" sections to reflect the latest state.
3. Maintain the YAML frontmatter. Update the `updated` field to today's date ({datetime.now().strftime('%Y-%m-%d')}).
4. Ensure bi-directional links (e.g., [[Entity Name]]) are preserved or added if relevant.
5. Keep the output as a single valid Markdown file.
6. List specific records/evidence under a "## Records" or "## History" section.

Output ONLY the updated Markdown.
"""
        system_prompt = "You are a professional industrial knowledge synthesizer. Respond only with the updated Markdown content."
        ...
        for email_id in email_ids:
            try:
                ...
                if row:
                    ...
                    print(f"[OK] 已分析郵件 ID: {email_id}")
                
            except Exception as e:
                print(f"[ERROR] 分析郵件 ID {email_id} 時發生錯誤: {e}")
                continue
...
"""
    ...
    def query_knowledge_base(self, query):
        """知識庫問答 (RAG) - 核心邏輯：檢索 Wiki (知識圖譜) + 統計數據 + 原始郵件"""
        # 1. 動態數據檢索 (統計 RAG)
        stats_context = ""
        try:
            conn = sqlite3.connect('emails.db')
            ...
            keywords = ["客訴", "投訴", "Complaint", "Claim", "異常", "維修", "退貨"]
            found_kws = [kw for kw in keywords if kw in query]
            if year or found_kws:
                sql = "SELECT count(*) FROM emails WHERE 1=1"
                params = []
                if year:
                    sql += " AND received_time LIKE ?"
                    params.append(f"{year}%")
                if found_kws:
                    sql += " AND (" + " OR ".join(["subject LIKE ?" for _ in found_kws]) + ")"
                    for kw in found_kws: params.append(f"%{kw}%")
                cursor.execute(sql, params)
                count = cursor.fetchone()[0]
                stats_context = f"\n【資料庫實時統計數據】：\n- 篩選條件: {year if year else '不限年份'} & {'/'.join(found_kws) if found_kws else '關鍵字'}\n- 找到的相關郵件總數: {count} 封\n"
            conn.close()
        except Exception as e: print(f"Stats query failed: {e}")
        ...
        if os.path.exists(wiki_root):
            ...
            for root, dirs, files in os.walk(wiki_root):
                for file in files:
                    # 跳過索引與系統檔案
                    if file.endswith('.md') and file.lower() not in ['index.md', 'log.md', '.processed_emails.txt']:
                        # 簡單關鍵字匹配 (檔名或內容)
                        if any(kw.lower() in file.lower() for kw in query.split()):
        ...
        # 3. 組合系統 Prompt (含 2026 時空座標)
        now = datetime.now().strftime('%Y-%m-%d')
        system_prompt = (
            f"你是一個資深工業專家智庫分析師。現在的日期是 {now}。\n"
            "你正在處理 Outlook 郵件知識庫。\n"
            f"{stats_context}\n"
            "【檢索背景資料】:\n"
            f"{wiki_context}\n"
            "請整合統計數據與實體內容回答問題。如果資料不足，請建議搜尋關鍵字。"
        )
        
    # 4. 呼叫 AI 引擎
        return self.call_ai(f"User Question: {query}", system_prompt)

    def get_entity_context(self, entity_name):
        """從資料庫動態獲取實體背景 (當 Wiki 檔案為空時的備案)"""
        try:
            conn = sqlite3.connect('emails.db')
            cursor = conn.cursor()
            # 搜尋主旨或內容包含該實體的郵件
            ...
            report = f"### 📊 實時資料庫脈絡：{entity_name}\n\n"
            report += f"偵測到與此實體相關的最近 {len(rows)} 封郵件：\n\n"
            
            for subject, sender, time, body in rows:
                clean_body = body[:250].replace('\n', ' ') + "..."
                display_time = time if time else "日期不詳"
                display_sender = sender if sender and sender != 'Unknown' else "系統分析實體"
                report += f"**[{display_time}] {display_sender}**: {subject}\n"
                report += f"> {clean_body}\n\n"
            
            report += "\n---\n*註：以上內容由 SkillsBuilder AI 從原始郵件庫動態提取。*"
            return report
        except Exception as e:
            return f"動態檢索失敗: {str(e)}"
